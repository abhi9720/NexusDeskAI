...
const Sidebar = (props: SidebarProps) => {
    const { isCollapsed, onToggle, lists, tasks, notes, savedFilters, activeSelection, onActiveSelectionChange, onOpenListModal, onDeleteSavedFilter, onDetailItemChange, userName, userStats, isMobileOpen, onMobileClose, onOpenSearch, onQuickAddTask, isAiLoading } = props;
    
    const handleOpenAddModal = (type: 'task' | 'note', parentId: number | null = null) => {
        onOpenListModal({ defaultType: type, defaultParentId: parentId });
    };
    ...
    const NavItem = ({ icon, label, count, isActive, onClick, onEdit, onDelete, shortcut }: { icon: React.ReactNode, label: string, count?: number, isActive: boolean, onClick: () => void, onEdit?: (e: React.MouseEvent) => void, onDelete?: (e: React.MouseEvent) => void, shortcut?: string }) => (
        <div className="group flex items-center w-full relative" title={isCollapsed ? `${label}${shortcut ? ` (⌘${shortcut})` : ''}` : undefined}>
            ...
            >
                ...
                {!isCollapsed && (
                    <div className="ml-auto flex-shrink-0 flex items-center gap-2">
                        {count !== undefined && (
                            <span className={`text-xs font-mono bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full px-2 py-0.5 transition-opacity ${shortcut ? 'group-hover:opacity-0' : ''}`}>{count}</span>
                        )}
                        {shortcut && (
                            <kbd className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity px-1.5 py-0.5 border border-gray-300 dark:border-gray-600 rounded-md font-mono text-xs text-gray-400 dark:text-gray-500">
                                ⌘{shortcut}
                            </kbd>
                        )}
                    </div>
        ...
        <aside className={`bg-container dark:bg-container-dark flex flex-col p-3 border-r border-gray-200 dark:border-gray-800/80 transition-transform duration-300 ease-in-out md:relative md:transform-none md:transition-all ${isCollapsed ? 'md:w-20' : 'md:w-72'} ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 left-0 z-50 w-72`}>
            ...
            <nav className="flex-1 space-y-2 overflow-y-auto pr-1 -mr-2">
                <div className="space-y-1">
                     <button 
                      ...
                      title={isCollapsed ? 'Prodify AI Chat (⌘3)' : undefined}
                      className={`group w-full flex items-center space-x-3 px-3 py-2 text-sm rounded-lg transition-colors duration-150 mb-2 font-semibold ${isCollapsed ? 'justify-center' : ''} bg-primary text-white shadow-lg shadow-primary/20 hover:bg-primary-dark`}
                     >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                          <SparklesIcon className="w-5 h-5" />
                          {!isCollapsed && <span className="truncate">Prodify AI Chat</span>}
                      </div>
                       {!isCollapsed && <kbd className="hidden group-hover:block px-1.5 py-0.5 border border-purple-300 rounded-md font-mono text-xs text-purple-200">⌘3</kbd>}
                    </button>
                    <NavItem icon={<DashboardIcon className="w-5 h-5"/>} label="Home" shortcut="1" isActive={activeSelection.type === 'dashboard'} onClick={() => handleSelection({type: 'dashboard'})}/>
                    ...
                    <NavItem icon={<TasksIcon className="w-5 h-5"/>} label="Today" shortcut="2" count={isCollapsed ? undefined : tasksTodayCount} isActive={activeSelection.type === 'smart-list' && activeSelection.id === 'today'} onClick={() => handleSelection({type: 'smart-list', id: 'today'})}/>
                    ...
                    <NavItem icon={<FlagIcon className="w-5 h-5" />} label="Goals" shortcut="4" isActive={activeSelection.type === 'goals'} onClick={() => handleSelection({type: 'goals'})} />
                    <NavItem icon={<CheckBadgeIcon className="w-5 h-5" />} label="Habits" shortcut="5" isActive={activeSelection.type === 'habits'} onClick={() => handleSelection({type: 'habits'})} />
            ...
            <div className="flex-shrink-0 pt-4 border-t border-gray-200 dark:border-gray-700/50 space-y-3">
                ...
                <button 
                    ...
                    className={`group flex items-center w-full p-2 rounded-lg text-left ${isCollapsed ? 'justify-center' : ''} hover:bg-gray-200/50 dark:hover:bg-gray-700/50`}
                    title={isCollapsed ? 'Settings (⌘6)' : ''}
                >
                    ...
                    {!isCollapsed && 
                        <div className="ml-3 flex-1 min-w-0 flex justify-between items-center">
                            ...
                            <kbd className="hidden group-hover:block px-1.5 py-0.5 border border-gray-300 dark:border-gray-600 rounded-md font-mono text-xs text-gray-400 dark:text-gray-500">⌘6</kbd>
                        </div>
                    }
                </button>
            </div>
